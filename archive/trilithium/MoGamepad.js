MoGamepad = Class.create(MoEventDispatcher, {
	initialize : function($super) {
		...
		if(MoIsNativeHost())
		{
			engine.onKey = this.onTrilithiumKey.bind(this);
			engine.onAnalogMove = this.onTrilithiumAnalogStickMove.bind(this);
		}
		else
		{
			if(MoIsFirefox())
		...
		this.connectionStatusTimer = new MoTimer(500);
		this.connectionStatusTimer.addEventHandler(MoTimerEvent.TICK, this.onConnectionStatusTimerTick.d(this));
		this.connectionStatusTimer.start();
	},
	
	onTrilithiumKey : function(key) {
		// only process controller keys
		if(key.deviceType != "controller")
			return;

		if(this.getIsRemoteControlKey(key.name))
		{
			var me = this;
			
			if(MoIsNull(key.onEnd))
			{			
				key.onEnd = (function() {
					me.onRemoteControlKeyEnd(this);
				}).bind(key);
			}
		}
			
		var idx = key.deviceId;
		var buttonIndex = this.getTrilithiumButtonIndex(key.name);
		var gamepads = MoGamepads(idx+1);
		var gp = null;

		if(MoIsNull(gamepads[idx]))
		{
			gamepads[idx] = {
				id : "PS3 Controller",
				index : idx,
			...
			this.onConnectionStatusTimerTick(null);
		...
		{
			gp = gamepads[idx];
			gp.timestamp = new Date();
		}

		if(buttonIndex != -1)
			gp.buttons[buttonIndex] = 1;

			
		// hook up events to listen for key release and 
		// pressure changes in triggers
		var me = this;
		
		if(MoIsNull(key.onEnd))
		{			
			key.onEnd = (function() {
				me.onTrilithiumKeyEnd(this);
			}).bind(key);
		}
		
		if(MoIsNull(key.onPressureChange) && this.getIsTrilithiumPressureKey(key.name))
		{
			key.onPressureChange = (function(value) {
				me.onTrilithiumKeyPressureChange(this, value);
			}).bind(key);
		}
	},
	
	onRemoteControlKeyEnd : function(key) {
		MoApplication.getInstance().handleRemoteControlKey(key);
	},

	onTrilithiumKeyEnd : function(key) {
		var idx = key.deviceId;
		var buttonIndex = this.getTrilithiumButtonIndex(key.name);
		var gamepads = MoGamepads(idx+1);
		var isFirstTime = (gamepads[idx].timestamp == 0);
		
		if(buttonIndex != -1)
			gamepads[idx].buttons[buttonIndex] = 0;

		gamepads[idx].timestamp = MoGetTimer();
		
		if(isFirstTime)
			this.onEventTimerTick(null);
	},

	onTrilithiumKeyPressureChange : function(key, value) {
		var idx = key.deviceId;
		var buttonIndex = this.getTrilithiumButtonIndex(key.name);
		var gamepads = MoGamepads(idx+1);

		if(buttonIndex != -1)
			gamepads[idx].buttons[buttonIndex] = value;

		gamepads[idx].timestamp = MoGetTimer();
	},
	
	onTrilithiumAnalogStickMove : function(id, x, y) {
		// there is no way to know which gamepad this came from
		// so we just update any non-null gamepad, fucking stooopid!

		var gamepads = MoGamepads();
		var xIndex = (id * 2) + 0;
		var yIndex = (id * 2) + 1;

		for(var i = 0, len = gamepads.length; i < len; ++i)
		{
			if(MoIsNull(gamepads[i]))
				continue;

			var px = gamepads[i].axes[xIndex];
			var py = gamepads[i].axes[yIndex];

			// just update both if any values have changed
			if(px != x || py != y)
			{
				gamepads[i].axes[xIndex] = x;
				gamepads[i].axes[yIndex] = y;
				gamepads[i].timestamp = MoGetTimer();
			}
		}
	},
	...
	getIsRemoteControlKey : function(keyId) {		
		switch(keyId)
		{
	...
	getTrilithiumButtonIndex : function(keyId) {	
		switch(keyId)
		{
			...
			case engine.keymap.controllerPS3.R2:
				return 7;
			case engine.keymap.controllerPS3.SELECT:
				return 8;
			case engine.keymap.controllerPS3.START:
				return 9;
			case engine.keymap.controllerPS3.L3:
				return 10;
			case engine.keymap.controllerPS3.R3:
				return 11;
	...
	getIsTrilithiumPressureKey : function(keyId) {
		return (keyId == engine.keymap.controllerPS3.L2 || keyId == engine.keymap.controllerPS3.R2);
	},

	getIsConnected : function(index) {
		var idx = index-1;
		
		if(idx < this.connected.length)
			return this.connected[idx];
	},
	
	getLastState : function(index) {
		var idx = index-1;
			
		if(idx >= this.connected.length || !this.connected[idx])
			return null;

		return this.prevStates[idx];
	},
	...
	getState : function(index) {
		...
		this.timestamps = this.syncArray(this.timestamps, 0);
		this.states = this.syncArray(this.states, null);
		this.prevStates = this.syncArray(this.prevStates, null);
			
		var idx = index-1;
		var gp = MoGamepads(index)[idx];
	...
	syncArray : function(src, defaultValue) {
		var gamepads = MoGamepads();
		
		if(gamepads.length != src.length)
		{
			var arr = [];
	...
	disableEvents : function() {
		...
		this.eventTimer = null;
		this.eventTicks = this.syncArray(this.eventTicks, 0);
	},
	...
	onEventTimerTick : function(e) {
		...
		for(var i = 0; i < gamepads.length; ++i)
		{
			lastState = this.getLastState(i+1);
			state = this.getState(i+1);

			// gamepad disconnected
			if(state == null || (state.timestamp == lastState.timestamp))
				continue;
			...
			//
			lastFrameTick = this.eventTicks[i];
			includeDownEvents = (lastFrameTick == 0 || (tickNow - lastFrameTick) >= 150);
			
			// process any events
			if(this.processStateEvents(i+1, lastState, state, includeDownEvents))
				this.eventTicks[i] = tickNow;
		}
	},
	...
	onConnectionStatusTimerTick : function(e) {
		var gamepads = MoGamepads();

		this.connected = this.syncArray(this.connected, false);
		
		for(var i = 0; i < gamepads.length; ++i)
		{
			var oldValue = this.connected[i];
			var newValue = !MoIsNull(gamepads[i]);
