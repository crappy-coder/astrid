...
// @PRIVATE 
{
	...
	unregisterKeyboardEvents : function() {
	...
	unregisterGamepadEvents : function() {
		var gp = MoGamepad.getInstance();
		
		gp.removeEventHandler(MoGamepadButtonEvent.DOWN, this.handleGamepadButtonDown.d(this));
		gp.removeEventHandler(MoGamepadButtonEvent.UP, this.handleGamepadButtonUp.d(this));
	},
		...
		if(target != this.focusTarget)
		{
			if(!isMouse || (isMouse && !MoIsNull(target) && target.getIsMouseFocusEnabled()))
			{
				...
				if(!MoIsNull(target))
				{
					...
					var nextTarget = this.focusTarget;
					
					// focus in on the target as long as the user didn't cancel
					if(nextTarget.handleEvent(new MoEvent(MoEvent.FOCUS_IN, false, true)))
						nextTarget.setIsFocused(true);
				}
			}
		}
	},
	...
	handleGamepadButtonDown : function(e) {
		...
		if(this.isNavigationButton(e.getButton()))
		{
