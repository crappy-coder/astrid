...
function handleApplicationStart(e) {
	...
	surface.addEventHandler(MoMouseEvent.MOUSE_DOWN, function(e) {
		var viewportCenter = content.getCenter();
		
		console.log(e.getX() - viewportCenter.x, -e.getY() + viewportCenter.y);
	});
...
MoIKBoneData = Class.create({
	initialize : function(length, angle) {
		this.length = length;
		this.angle = MoMath.degreesToRadians(angle);
	}
});
