 ...
 */
class EventDispatcher extends Equatable {
	constructor() {
		super();
		this.handlerCount = 0;
	}
	...
	determinePropagationPathForTarget(target) {
		// @PRIVATE
	...
	executeEventHandlers(event) {
		...
		if (queue != null) {
			...
			for (i = 0; i < len; ++i) {
				...
				if (!useCapture && event.getPhase() == EventPhase.CAPTURING) continue;

				// or move on to the next one if we are currently bubbling and
				// the handler should only be used during the capture phase
				if (useCapture && event.getPhase() == EventPhase.BUBBLING) continue;
	...
	getQueueName$internal(eventType) {
		// @PRIVATE

		return "__evt_queue_" + eventType;
	}

	static __removeEventListener(queue, handler, useCapture) {
		// @PRIVATE
