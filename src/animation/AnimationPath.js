...
/**
 * SUMMARY:
 *  The AnimationPath class represents a collection of Keyframe objects for an animation and the name of the
 ...
 *
 */
class AnimationPath extends Equatable {
	constructor(property) {
		/**
		 * SUMMARY:
		 *  Initializes a new instance of the AnimationPath class with the specified property.
		 *
		 * PARAMS:
		 *  String property:
	...
	getInterpolator() {
		 ...
		 *  then the default NumberInterpolator is returned.
		 *
	...
	getKeyframes() {
		/**
		 * SUMMARY:
		 *  Gets an array of Keyframe objects that represent the time/value pairs
		 *  that the property will take during animation.
		 *
		 * RETURNS (Keyframe[]):
		 *  A reference to the Keyframe array.
		 */
	...
	validateKeyframesForDuration(duration) {
		// @PRIVATE
		var keyframes = this.getKeyframes();
		var len = keyframes.length;
