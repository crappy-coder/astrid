...
MoRTree = Class.create({
	initialize : function(size) {
		this.minWidth = MoIsNull(size) ? 3 : Math.floor(size * 0.5);
		this.maxWidth = MoValueOrDefault(size, 6);
	...
	overlapRectangle : function(a, b) {
		return(a.x < (b.x+b.w) && (a.x+a.w) > b.x && a.y < (b.y+b.h) && (a.y+a.h) > b.y);
	},
	
	containsRectangle : function(a, b) {
		return((a.x+a.w) <= (b.x+b.w) && a.x >= b.x && (a.y+a.h) <= (b.y+b.h) && a.y >= b.y);
	},
	
	expandRectangle : function(a, b) {
	...
	squarifiedRatio : function(l, w, fill) {
	...
	removeSubtree : function(rect, obj, root) {
	...
	chooseLeafSubtree : function(rect, root) {
	...
	searchSubtree : function(rect, return_node, return_array, root) {
		var hit_stack = []; // Contains the elements that overlap
